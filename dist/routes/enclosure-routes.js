...
enclosureRouter.patch("/:id", async (c) => {
    ...
    try {
        const result = await EnclosureController.updateEnclosure(updateEnclosure, id);
        return c.json({ message: "Gehege wurde erfolgreich aktualisiert.", result }, 200);
    }
    catch (error) {
        return c.json({ message: "Fehler beim Aktualisieren des Geheges.", error }, 500);
    }
});
enclosureRouter.delete("/:id", async (c) => {
    ...
    try {
        ...
        return c.json({ message: `Gehege mit ID: ${id} wurde erfolgreich gelöscht` }, 200);
    }
    catch (error) {
        return c.json({ message: `Fehler beim Löschen des Geheges ${id}` }, 500);
    }
