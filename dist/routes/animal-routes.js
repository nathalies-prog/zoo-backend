...
animalRouter.post("/insertAnimal", async (c) => {
    const { breed, name, birthday, gender, health, vet_id, enclosure_id } = await c.req.json();
    try {
...
animalRouter.patch("/:id", async (c) => {
    ...
    try {
        ...
        return c.json({ message: "Fehler beim Aktualisieren des Tieres.", error }, 500);
    }
});
...
animalRouter.post('/:id/assign-enclosure', async (c) => {
    ...
    const animalQueryResult = await getPool().query('SELECT * FROM "Animal" WHERE Id = $1', [animalId]);
    ...
    if (isNaN(compoundId)) {
        throw new HTTPException(400, {
            message: "Gehege_id ist kein Type Integer",
        });
    }
    const compoundQueryResult = await getPool().query('SELECT * FROM "Enclosure" WHERE Id = $1', [compoundId]);
    ...
    const updateResult = await getPool().query('UPDATE "Animal" SET enclosure_id = $1 WHERE id = $2 RETURNING *', [compoundId, animalId]);
