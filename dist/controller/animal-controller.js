...
export class AnimalController {
    ...
    static async insertAnimal(animal) {
        const { breed, name, birthday, gender, health, vet_id, enclosure_id } = animal;
        try {
          ...
          `, [vet_id]);
            ...
            if (count >= 25) {
                throw new Error("Der ausgewählte Tierarzt betreut bereits 25 Tiere und kann kein weiteres Tier übernehmen.");
            }
            const result = await getPool().query(`
    ...
    static async updateAnimal(animal, id) {
        const result = await getPool().query(`UPDATE "Animal" SET breed=$1, name=$2,birthday=$3,gender=$4,health=$5,vet_id=$6,enclosure_id=$7 WHERE id = $8 `, [
            animal.breed,
            animal.name,
            animal.birthday,
            animal.gender,
    ...
    static async deleteAnimal(id) {
        try {
            const result = await getPool().query(`DELETE FROM "Animal" WHERE id = $1`, [id]);
            return result.rows[0];
        }
        catch (error) {
            console.error(`Irgendwas ist beim Löschen schief gelaufen = Animal: ${id}`, error);
        }
    }
}
