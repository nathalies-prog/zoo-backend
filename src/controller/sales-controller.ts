...
export class SalesStandController {
  ...
  static async getSalesStandByID(id: string) {
    try {
      const result = await getPool().query(
        `SELECT * FROM "SalesStand" WHERE id = $1`,
        [id]
      );
      return result.rows[0];
    } catch (error) {
      console.error(
        `Fehler beim Abrufen des Verkaufsstandes mit ID ${id}:`,
        error
      );
      throw error;
    }
  }
  static async recordSale(salesStandId: string, revenue: number) {
    try {
      const salesStand = await getPool().query(
        `SELECT * FROM "SalesStand" WHERE id = $1`,
        [salesStandId]
      );
      ...
      if (!salesPersonId) {
        throw new Error(
          "Es muss ein Verkäufer zugewiesen werden, um Verkäufe zu erzielen."
        );
      }
      ...
      await getPool().query(
        `UPDATE "SalesStand" SET revenue_per_day = revenue_per_day + $1 WHERE id = $2`,
        [revenue, salesStandId]
      );
